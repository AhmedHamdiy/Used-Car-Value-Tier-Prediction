...
def transform_data(df_kaggle, df_crawled):
    # 1. brand col
    df_kaggle['brand'] = (df_kaggle['brand'].astype(str).str
                          .lower().str.replace(r'[-_]', '', regex=True))
    df_crawled['brand'] = (df_crawled['brand'].astype(str).str
                           .lower().str.replace(r'[-_]', '', regex=True))
    ...
    df_kaggle['fuelType'] = (df_kaggle['fuelType'].str
                             .lower().map(fuel_map_kaggle))
    df_crawled['fuelType'] = (df_crawled['fuelType']
                              .map(fuel_map_crawled))

    # 3. kilometer/mileage col
    df_crawled = df_crawled.rename(columns={'mileage': 'kilometer'})
    df_crawled['kilometer'] = (pd.to_numeric(df_crawled['kilometer'],
                                             errors='coerce').astype('Int64'))

    # 4. year col
    df_crawled['yearOfRegistration'] = (df_crawled['year'].astype(str).str
                                        .extract(r'(\d{4})').astype(float))
    df_kaggle['yearOfRegistration'] = (pd.to_numeric(
        df_kaggle['yearOfRegistration'], errors='coerce'))

    # 5. vehicleType col
    vt_map_kaggle = {
        'limousine': 'sedan', 'kleinwagen': 'compact',
        'kombi': 'station_wagon', 'bus': 'van',
        'cabrio': 'convertible', 'coupe': 'coupe',
        'suv': 'suv', 'andere': 'other'
    }
    vt_map_crawled = {
        'Compact': 'compact', 'Station Wagon': 'station_wagon',
        'SUV/Off-Road/Pick-Up': 'suv', 'Sedan': 'sedan',
        'Van': 'van', 'Convertible': 'convertible',
        'Coupe': 'coupe', 'Other': 'other'
        }
    df_kaggle['vehicleType'] = (df_kaggle['vehicleType'].str
                                .lower().map(vt_map_kaggle))
    df_crawled['vehicleType'] = (df_crawled['vehicleType']
                                 .map(vt_map_crawled))

    # 6. seller col
    seller_map_kaggle = {'privat': 'private', 'gewerblich': 'dealer'}
    seller_map_crawled = {'Dealer': 'dealer'}
    df_kaggle['seller'] = (df_kaggle['seller']
                           .str.lower()
                           .map(seller_map_kaggle))
    df_crawled['seller'] = (df_crawled['seller'].
                            map(seller_map_crawled))
    ...
    # 8. model col
    df_kaggle['model'] = (df_kaggle['model'].astype(str).str
                          .lower().str.replace(r'[-_ ]', '', regex=True))
    df_crawled['model'] = (df_crawled['model'].astype(str).str
                           .lower().str.replace(r'[-_ ]', '', regex=True))
    ...
    df_kaggle['gearbox'] = df_kaggle['gearbox'].map(gear_box_map)
    df_crawled['gearbox'] = df_crawled['gearbox'].str.lower()

    # 10. Target columns and merge
    target_cols = [
