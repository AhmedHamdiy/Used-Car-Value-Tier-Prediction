...
class CarsSpider(scrapy.Spider):
    ...
    def start_requests(self):
        """Kick off by requesting listing pages (Splash for listing, plain HTTP for details)."""
    ...
    def parse_listing(self, response):
        ...
        if next_data_text:
            try:
                ...
                self.logger.warning(
                    "Page %s: could not decode __NEXT_DATA__ JSON.", page
                )

        # Fallback strategy: parse rendered anchors
        if not links:
            links = response.css('a[href*="/offers/"]::attr(href)').getall()
        ...
        for href in links:
            ...
            # Detail pages already contain required fields in HTML, so normal Request is much faster.
            yield scrapy.Request(
                url=full_url,
    ...
    def parse_car(self, response):
        ...
        # ── Overview quick-facts (mileage, gearbox, power, fuel, year, seller)
    ...
    def _get_overview(self, response, label: str) -> str:
        ...
        We find the title matching `label` then grab the next value div's text.
        """
        titles = response.css("div.VehicleOverview_itemTitle__S2_lb")
        for title in titles:
            if title.css("::text").get("").strip().lower() == label.lower():
                # The value div is the next sibling after the empty separator div
    ...
    def _get_detail_field(self, response, label: str) -> str:
        """
        In the Basic Data section, fields are <dt>label</dt><dd>value</dd> pairs.
        """
        dts = response.css("dt")
        for dt in dts:
